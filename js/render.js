...
(function () {
  ...
  function formUrl(lang) {
    var u = (C.formUrl && (C.formUrl[lang] || C.formUrl.pl)) || "";
    return isTodo(u) ? "" : u;
  }
  ...
  function renderHeader(T, lang) {
    ...
    document.getElementById("brand").innerHTML = logoZ + '<span class="brand-text">' + esc(T.nav.brand) + "</span>";
  ...
  function renderHero(T) {
    ...
    document.getElementById("hero-content").innerHTML =
      ...
      (C.invitePdf && h.invite ? '<p class="hero-link"><a href="' + esc(C.invitePdf) + '" target="_blank" rel="noopener">' + icon("external") + esc(h.invite) + "</a></p>" : "");
  }
  ...
  function renderSpeakers(T) {
    ...
    var orgs = O.items.map(function (o) {
      var logo = o.logo === "ziarno" && C.logoZiarno ? '<img src="' + esc(C.logoZiarno) + '" alt="">' : esc(o.name.split(" ")[0]);
  ...
  function renderRegister(T, lang) {
    ...
    var pkgs = PR.packages.map(function (p) {
      ...
      return '<div class="pkg' + (p.badge ? " featured" : "") + '">' + (p.badge ? '<span class="badge">' + esc(p.badge) + "</span>" : "") +
        '<div class="name">' + esc(p.name) + '</div><div class="amount">' + price.eur + " EUR<small>" + esc(PR.perPerson) + "</small></div>" + (price.pln ? '<div class="pln">≈ ' + price.pln + " PLN</div>" : "") +
    ...
    if (regState()) {
      ...
      var form = url ? '<div class="form-embed"><iframe title="' + esc(R.open.title) + '" src="' + esc(url + (url.indexOf("?") > -1 ? "&" : "?") + "embedded=true") + '" loading="lazy"></iframe></div>' : '<p class="todo">' + esc(R.open.pending) + "</p>";
      ...
      box = '<div class="register-box" id="form"><h3>' + esc(R.open.title) + '</h3><p class="lead">' + esc(R.open.lead) + "</p>" + meta + form + actions + "</div>";
    } else {
  ...
  function renderFooter(T) {
    ...
    document.getElementById("footer").innerHTML =
      '<div class="wrap"><div><div class="f-brand">' + logoZ + '</div><div class="f-title">' + esc(T.hero.title) + " 2027</div><p>" + esc(F.org) + "<br>" + esc(F.address) + "</p></div>" +
      ...
      "<div><h3>" + esc(F.contactTitle) + '</h3><ul><li><a href="mailto:' + esc(C.contactEmail) + '">' + esc(C.contactEmail) + '</a></li><li><a href="https://www.ziarno.edu.pl/" target="_blank" rel="noopener">ziarno.edu.pl</a></li></ul></div>' +
      ...
      '<div class="bottom-bar">' + esc(F.copy) + ' · <a href="' + esc(F.privacyUrl) + '">' + esc(F.privacy) + "</a>" +
  ...
  window.renderSite = function (lang) {
    ...
    renderHeader(T, lang); renderHero(T); renderWhy(T); renderWho(T); renderProgram(T); renderSpeakers(T); renderPartners(T); renderPractical(T, lang); renderRegister(T, lang); renderFooter(T);
    afterRender(T, lang);
  };
